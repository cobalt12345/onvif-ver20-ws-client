package ru.sitronics.onvif.soap;

import ru.sitronics.antidrone.onvif.device.Capabilities;
import ru.sitronics.antidrone.onvif.device.CapabilityCategory;
import ru.sitronics.antidrone.onvif.device.Device;
import ru.sitronics.antidrone.onvif.device.DeviceService;
import ru.sitronics.antidrone.onvif.events.EventPortType;
import ru.sitronics.antidrone.onvif.events.EventService;
import ru.sitronics.antidrone.onvif.imaging.ImagingPort;
import ru.sitronics.antidrone.onvif.imaging.ImagingService;
import ru.sitronics.antidrone.onvif.media.ver10.*;
import ru.sitronics.antidrone.onvif.ptz.PTZ;
import ru.sitronics.antidrone.onvif.ptz.PtzService;
import ru.sitronics.onvif.beans.DeviceInfo;
...
import ru.sitronics.antidrone.onvif.device.DateTime;
...
public class OnvifDevice {
  ...
  protected void init() throws ConnectException, SOAPException {
    ...
    if (capabilities.getMedia() != null && capabilities.getMedia().getXAddr() != null) {
      this.media = new MediaService().getDevicePort();
      this.media =
    ...
    if (capabilities.getPTZ() != null && capabilities.getPTZ().getXAddr() != null) {
      this.ptz = new PtzService().getDevicePort();
      this.ptz =
    ...
    if (capabilities.getImaging() != null && capabilities.getImaging().getXAddr() != null) {
      this.imaging = new ImagingService().getDevicePort();
      this.imaging =
    ...
    if (capabilities.getEvents() != null && capabilities.getEvents().getXAddr() != null) {
      this.events = new EventService().getDevicePort();
      this.events =
...
//  }
  ...
  public String getSnapshotUri(String profileToken) {
    ru.sitronics.antidrone.onvif.media.ver10.MediaUri sceenshotUri = media.getSnapshotUri(profileToken);
  ...
  public String getStreamUri(String profileToken) {
    ...
    ru.sitronics.antidrone.onvif.media.ver10.MediaUri rtsp = media.getStreamUri(streamSetup, profileToken);
